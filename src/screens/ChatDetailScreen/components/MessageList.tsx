...
function buildCallLogLayout(m: Extract<ChatMessage, { type: 'call_log' }>): CallBubbleLayout {
  ...
  if (['MISSED', 'NO_ANSWER', 'CANCELLED'].includes(status))
    return {
...
function parseCallStateText(text: string, sent: boolean): CallBubbleLayout | null {
  ...
  if (lower.includes('missed video call'))
    return { title: 'Missed Video Call', subtitle: '', variant: 'missed', icon: 'videoMissed' };
  if (lower.includes('missed voice call'))
    return { title: 'Missed Voice Call', subtitle: '', variant: 'missed', icon: 'voiceMissed' };
  if (lower.includes('incoming voice call'))
    return { title: 'Incoming Voice Call', subtitle: 'Tap to receive', variant: 'received', icon: 'voiceIncoming' };
  if (lower.includes('incoming video call'))
    return { title: 'Incoming Video Call', subtitle: 'Tap to receive', variant: 'received', icon: 'videoIncoming' };
  if (lower.includes('voice call') && lower.includes('ring'))
    return { title: 'Voice Call', subtitle: 'Ringing..', variant: 'sent', icon: 'voiceOutgoing' };
  if (lower.includes('video call'))
    return {
      title: 'Video Call',
      subtitle: durationMatch?.[0] ?? agoMatch?.[0] ?? (sent ? '6 Secs' : '1 Min ago'),
      variant: sent ? 'sent' : 'received',
      icon: sent ? 'videoOutgoing' : 'videoIncoming',
    };
  if (lower.includes('voice call'))
    return {
      ...
      variant: sent ? 'sent' : 'received',
      icon: sent ? 'voiceOutgoing' : 'voiceIncoming',
    };
  return null;
}
...
export const MessageList = React.memo(function MessageList({
  ...
  const renderCallBubbleRow = useCallback(
    (callState: CallBubbleLayout, isSent: boolean, timestamp: string, keyIndex: number) => (
                ...
                >
                  {msg.blocks.map((block, blockIndex) => {
                    if (block.type === 'paragraph') {
                      return (
                        <Text key={`${index}_p_${blockIndex}`} style={[styles.bubbleText, msg.sent && styles.bubbleTextSent, { marginBottom: 8 }]}>
                          {block.text}
                        </Text>
                      );
                    }
        ...
        return (
          <React.Fragment key={index}>
            <View style={[styles.messageRow, msg.sent ? undefined : styles.messageRowReceived]}>
              <View ref={(r) => setMessageBubbleRef(index, r)} collapsable={false}>
                <TouchableOpacity
                  style={[styles.voiceBubbleSent, msg.sent ? undefined : styles.voiceBubbleReceived]}
                  activeOpacity={1}
                  onLongPress={() => handleMessageLongPress(index)}
                >
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                    onPress={() => {
                      void voice.toggleVoiceMessagePlayback(msg.uri, messageKey);
                    }}
                  >
                    {isPlayingNow ? (
                  ...
                  <View style={styles.voiceBubbleWaveform}>
                    {VOICE_WAVEFORM.map((h, i) => (
                      <View
                        key={i}
                        style={[styles.voiceBubbleWaveformBar, !msg.sent && styles.voiceBubbleWaveformBarReceived, { height: Math.max(6, h) }]}
                      />
                    ))}
                  </View>
                </TouchableOpacity>
