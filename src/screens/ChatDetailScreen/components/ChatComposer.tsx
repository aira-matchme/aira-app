...
import { styles, CHAT_INPUT_MIN_HEIGHT, CHAT_INPUT_MAX_HEIGHT } from '../styles';
...
interface ChatComposerProps {
  ...
  typingStopRef: React.MutableRefObject<ReturnType<typeof setTimeout> | null>;
  typingDebounceRef: React.MutableRefObject<ReturnType<typeof setTimeout> | null>;
  ...
  onTypingStart: () => void;
  onTypingStop: () => void;
  getMessagePreview: (msg: any) => string;
    ...
    <View onLayout={onLayout}>
      {replyingTo != null && (
        <View style={styles.replyToBar}>
          <View style={styles.replyToBarGradientBorder} />
          <View style={styles.replyToBarContent}>
            <Text style={styles.replyToBarLabel}>{STRINGS.CHAT.REPLYING_TO} {replyingTo.senderName}</Text>
            <Text style={styles.replyToBarPreview} numberOfLines={2}>{getMessagePreview(replyingTo.message)}</Text>
          </View>
          <TouchableOpacity style={styles.replyToBarDismiss} onPress={() => setReplyingTo(null)} activeOpacity={0.8}>
      ...
      <View style={[styles.inputBar, { paddingBottom: 12 + bottomSafeInset }]}>
                ...
                <View key={i} style={styles.attachmentPreviewWrapper}>
                  <View style={att.type === 'image' ? styles.attachmentPreview : styles.attachmentPreviewFileCard}>
                    {att.type === 'image' ? (
                      ...
                      <>
                        <Text style={styles.attachmentPreviewFileType}>{getFileTypeLabel((att as any).name)}</Text>
                        <Text style={styles.attachmentPreviewFileName} numberOfLines={2}>{(att as any).name}</Text>
                      </>
                    )}
                  </View>
                  <TouchableOpacity style={styles.attachmentRemove} onPress={() => setPendingAttachments((p) => p.filter((_, idx) => idx !== i))}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
          onPress={() => {
            if (!hasSendContent) { onMicPress(); return; }
            onSendPress();
          }}
